...
void SSocket::SetRecvBufSize(int iSize)
{
...
#else
	socklen_t sizelen = 4;
	//MEMO: 检测内核的最大值设置，如果不足则自动放大，避免设置不生效 [2015-10-26 13:57 邵凯田]
	...
	if(f.open(IO_ReadOnly))
	{
		...
		{
			LOGWARN("待设置的接收缓冲区大小[%d]小于内核允许的最大缓冲大小[%d],将自动放大该内核参数!",iSize,bytes);
...
void SSocket::SetSendBufSize(int iSize)
{
...
#else
	socklen_t sizelen = 4;
	//MEMO: 检测内核的最大值设置，如果不足则自动放大，避免设置不生效 [2015-10-26 13:58 邵凯田]
	...
	if(bytes > 0 && bytes < iSize)
	{
		LOGWARN("待设置的发送缓冲区大小[%d]小于内核允许的最大缓冲大小[%d],将自动放大该内核参数!",iSize,bytes);
...
bool SSocket::Accept(SSocket &newSocket)
{
	...
	int bNodelay = 1;  
	setsockopt(sock,IPPROTO_TCP,TCP_NODELAY,(char *)&bNodelay,sizeof(bNodelay));//不采用延时算法 
