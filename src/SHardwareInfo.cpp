/**
 *
 * 文 件 名 : SHardwareInfo.cpp
 * 创建日期 : 2017-7-4 11:10
 * 作    者 : 邵凯田(skt001@163.com)
 * 修改日期 : $Date: $
 * 当前版本 : $Revision: $
 * 功能描述 : 硬件信息获取类，含CPU标识、磁盘标识
 * 修改记录 : 
 *            $Log: $
 *
 * Ver  Date        Author  Comments
 * ---  ----------  ------  -------------------------------------------
 * 001	2017-7-4	邵凯田　创建文件
 *
 **/

#include "SHardwareInfo.h"

SHardwareInfo::SHardwareInfo()
{
...
SString SHardwareInfo::GetCpuSn()
{
	...
	*/
	...
	//printf("s3 s4 = %08X-%08X \n",s3,s4);     
#endif
	sn.sprintf("%08X-%08X-%08X-%08X",s1,s2,s3,s4);
	return sn;
}
...
// 返 回 值:  SString,空串表示失败
//////////////////////////////////////////////////////////////////////////
SString SHardwareInfo::GetDiskSn()
{
	SString sn;
#ifdef WIN32
	str_DN_Serial = "";
	...
	for(int i=0;i<4;i++)
	{
		SString sDiskName = "/dev/sd";
		sDiskName += char('a'+i);
		int fd = open(sDiskName.data(), O_RDONLY|O_NONBLOCK);
		//失败返回 
		if (fd < 0)
		{
			close(fd);
			LOGERROR("取磁盘[%s]标识失败!",sDiskName.data());
			continue;
		}
...
// 创建时间:  2017-7-3 19:02
// 参数说明:  void
// 返 回 值:  SString,空串表示不支持的硬件架构
//////////////////////////////////////////////////////////////////////////
SString SHardwareInfo::GetHostSn()
{
	SString cpu_sn = GetCpuSn();
	SString disk_sn = GetDiskSn();
	if(cpu_sn.length() == 0 && disk_sn.length() == 0)
		return "";
	return cpu_sn + "_"+disk_sn;
}
...
// 创建时间:  2017-7-3 19:04
// 参数说明:  void
// 返 回 值:  true表示授权通过，false表示授权拒绝
//////////////////////////////////////////////////////////////////////////
bool SHardwareInfo::CheckLicense()
{
	return false;
}

#ifdef WIN32
BOOL SHardwareInfo::ReturnInfo(int drive, DWORD diskdata [256])
{
	char string1 [1024];
   __int64 sectors = 0;
   __int64 bytes = 0;
...
//MessageBox(NULL,str_HardDesk_Form,NULL,NULL);
   ...
   str_DN_Modol =SString(ConvertToString (diskdata, 27, 46));
  
   str_DN_Serial =SString(ConvertToString (diskdata, 10, 19));

   str_DN_ControllerRevision =SString(ConvertToString (diskdata, 23, 26));

   str_HardDeskBufferSize.sprintf("%u", diskdata [21] * 512);
   ...
   if (diskdata [0] & 0x0080)
	...
	bytes = sectors * 512;
	str_HardDeskSize.sprintf("%I64d",bytes);

   m_sInfo += str_HardDesk_Form+"\r\n";
   m_sInfo += str_Controller+"\r\n";
   m_sInfo += str_DN_Modol+"\r\n";
   m_sInfo += str_DN_Serial+"\r\n";
   m_sInfo += str_DN_ControllerRevision+"\r\n";
   m_sInfo += str_HardDeskBufferSize+"\r\n";
   m_sInfo += str_HardDeskType+"\r\n";
   m_sInfo += str_HardDeskSize+"\r\n\r\n\r\n";

return 1;

}
//conversion to char 
char *SHardwareInfo::ConvertToString(DWORD diskdata [256], int firstIndex, int lastIndex)
{
...
//
 BOOL SHardwareInfo::DoIDENTIFY(HANDLE hPhysicalDriveIOCTL, PSENDCMDINPARAMS pSCIP,
...
//
int SHardwareInfo::ReadPhysicalDriveInNT(void)
{
	int done = FALSE;
   int drive = 0;
...
//
int SHardwareInfo::ReadDrivePortsInWin9X(void)
{
	int done = FALSE;
   HANDLE VxDHandle = 0;
...
SetPriorityClass (GetCurrentProcess (), REALTIME_PRIORITY_CLASS);
   ...
   {
	   LOGERROR("ERROR: Could not open IDE21201.VXD file!");
		//MessageBox (NULL, "ERROR: Could not open IDE21201.VXD file", 				"YSPAS", MB_ICONSTOP);
   }
...
//
int SHardwareInfo::ReadIdeDriveAsScsiDriveInNT(void)
{
	int done = FALSE;
   int controller = 0;
