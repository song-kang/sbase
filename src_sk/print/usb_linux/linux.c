...
int usb_os_find_devices(struct usb_bus *bus, struct usb_device **devices)
{
  ...
  while ((entry = readdir(dir)) != NULL) {
    ...
    for (i = 0; i < dev->descriptor.bNumConfigurations; i++) {
      ...
      bigbuffer = (unsigned char*)malloc(config.wTotalLength);
      if (!bigbuffer) {
        if (usb_debug >= 1)
...
int usb_os_determine_children(struct usb_bus *bus)
{
  ...
  for (dev = bus->devices; dev; dev = dev->next) {
    ...
    dev->children = (struct usb_device**)malloc(sizeof(struct usb_device *) * dev->num_children);
    if (!dev->children) {
      if (usb_debug > 1)
...
int usb_resetep(usb_dev_handle *dev, unsigned int ep)
{
  ...
  if (ret)
    USB_ERROR_STR(-errno, "could not reset ep %u: %s", ep,
    	strerror(errno));
...
int usb_clear_halt(usb_dev_handle *dev, unsigned int ep)
{
  ...
  if (ret)
    USB_ERROR_STR(-errno, "could not clear/halt ep %u: %s", ep,
    	strerror(errno));
