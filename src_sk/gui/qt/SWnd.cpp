...
void SWnd::paintEvent(QPaintEvent *event)
{
	S_UNUSED(event);
...
void SWnd::resizeEvent(QResizeEvent * event)
{
	S_UNUSED(event);
	OnSize(1,0,0);
}
...
bool SWnd::SendMessage(int message,void* wParam,void* lParam)
{
	if(this == NULL)
		return false;
	::SendMessage(m_hWnd,message,(WPARAM)wParam,(LPARAM)lParam);
	return true;
}
