...
struct SBASE_ORACLE_EXPORT OdbField
{
	...
	OCILobLocator** ppoLocator;					//LOB描述器
	int iRows;
    std::string   FileName;
    ...
    OdbField();							//构造
    ~OdbField();							//析构
    void SetSize(ub2 w,int once_rows=1);						//产生pBuffer的内存,大小 w
    void Set(std::string cs);					//将cs的值付给pBuffer
    std::string Get(int row=0) const;						//得到pBuffer的值,返回string
...
struct SBASE_ORACLE_EXPORT OdbRecordSet
{
    ...
    std::string GetFieldValue(int iField,int row=0);	//得到第几个字段的值
...
class SBASE_ORACLE_EXPORT COracleDataBase
{
    ...
    //查询数据库内容
    long Select(const std::string Stmt,OdbRecordSet &rs,long &hpSelect/*Stmt handle*/,int once_rows=0);			 //查询
    sword FetchNext(long hpSelect,OdbRecordSet &rs,int once_rows=1);    						 //取结果
