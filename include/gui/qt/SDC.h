...
class SBASE_QTGUI_EXPORT SDC : public SBaseDC
{
public:
...
// 		m_hDC->setPen(pen);
	...
	{
		//QFont font(p->szFace,(int)(((float)p->m_iHeight)/1.25),p->m_iWeight,p->fdwItalic==1);
		QFont font(p->szFace,(int)(((float)p->m_iHeight)*72/96),p->m_iWeight,p->fdwItalic==1);
		m_hDC->setFont(font);
		/*
	...
	{
		bool bHint = m_hDC->testRenderHint(QPainter::Antialiasing);
		m_hDC->setRenderHint(QPainter::Antialiasing,true);//设置平滑
		m_hDC->drawLine(m_x,m_y,x,y);
		m_hDC->setRenderHint(QPainter::Antialiasing,bHint);//设置平滑
		m_x = x;
		m_y = y;
		//::LineTo(m_hDC,x,y);
	}
	...
	{
		S_UNUSED(x2);S_UNUSED(y2);
		S_UNUSED(x3);S_UNUSED(y3);
	...
	{
		S_UNUSED(x);S_UNUSED(y);
		S_UNUSED(nRadius);S_UNUSED(fStartAngle);S_UNUSED(fSweepAngle);
	...
	virtual void Polygon(const YSPOINT* pts,int size)
	{
		...
		bool bHint = m_hDC->testRenderHint(QPainter::Antialiasing);
		m_hDC->setRenderHint(QPainter::Antialiasing,true);//设置平滑
		//m_hDC->fillPath(path,m_hDC->brush());
		m_hDC->drawPath( path);		
		m_hDC->setRenderHint(QPainter::Antialiasing,bHint);//设置平滑
